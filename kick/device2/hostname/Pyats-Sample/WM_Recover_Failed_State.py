"""
This is pyats test script to recover the WM device from failed state
...
class DropToRommon(aetest.Testcase):
    ...
    @aetest.test
    def baseline(self):
        """ Trigger the Baseline from rommon prompt
        """
        global wm, dev
        wm = Wm(hostname=HOSTNAME, login_username=DEVICE_USERNAME,
        ...
        '''
        if CONNECTION_TYPE is not None and CONNECTION_TYPE == "ssh":
            dev = wm.ssh_console(ip=SSH_IP, port=SSH_PORT, username=USERNAME,
                                  password=PASSWORD)
        elif CONNECTION_TYPE is not None and CONNECTION_TYPE == "telnet":
            dev = wm.telnet_console(ip=SSH_IP, port=SSH_PORT, username=USERNAME,
                                     password=PASSWORD)
        else:
            dev = None
        '''
